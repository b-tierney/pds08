...
ggsave('bifido_change_across_tp.pdf',width=30,height=15)
...
output = list()
for(r in regcolumns){
  output[[r]] = glm(data = abdata_b,family='binomial', RESP_STATUS ~ age+ (abdata_b[,r])) %>% tidy %>% mutate(microbial_var = r) %>% filter(term!='(Intercept)',term!='age')
}
