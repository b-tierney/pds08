...
melted_meta %>% arrange(value)

ggplot(melted_meta %>% arrange(value),aes(x=RESP_STATUS,y=value)) + geom_point(aes(color=RESP_STATUS)) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1))
...
#### NEED TO CHECK IF HOLDS UP FOR ALTERNATIVE TAXONOMIC METHOD 
...
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='richness') ,value ~ RESP_STATUS)
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='shannon') ,value ~ RESP_STATUS)
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='simpson') ,value ~ RESP_STATUS)

ggplot(data = abdata_div_melted, aes(x = as.factor(timepoint), y = value)) + geom_boxplot()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +stat_compare_means(paired = T,method='wilcox.test')+ geom_point(aes(fill=as.factor(timepoint),group=Sample_ID), position = position_dodge(0.2)) + geom_line(aes(group=Sample_ID),color='grey',position = position_dodge(0.2)) + facet_grid(cols=vars(IMPROVED),rows=vars(variable),scales='free')
...
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='richness') ,value ~ IMPROVED)
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='shannon') ,value ~ IMPROVED)
compare_means(data = abdata_div_melted %>% filter(timepoint == 'baseline') %>% filter(variable=='simpson') ,value ~ IMPROVED)
...
ggplot(data = abdata_bif_melted_all_bif, aes(x = as.factor(timepoint), y = log(value+0.00001))) + geom_boxplot()+ theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) + geom_point() + geom_line(aes(group=Sample_ID),color='grey') + facet_grid(cols=vars(RESP_STATUS)) + stat_compare_means(paired=T,method='wilcox.test')
...
output = list()
for(r in regcolumns){
  output[[r]] = multinom(data = abdata_b, RESP_STATUS ~ age+ log(abdata_b[,r]+0.00001),family='binomial') %>% tidy %>% mutate(microbial_var = r %>% strsplit('s__') %>% map_chr(2)) %>% filter(term!='(Intercept)',term!='age')
}
...
microbes_of_interest = output_merged %>% filter(adj<0.05,y.level == 'RESPONDER') %>% arrange(desc(abs(estimate))) %>% select(microbial_var) %>% unlist %>% unname %>% unique

for(m in microbes_of_interest){
  abdata_b %>% select(RESP_STATUS,all_of(grep(m,colnames(abdata_b)))) %>% filter(RESP_STATUS == 'RESPONDER' | RESP_STATUS == 'NOCHANGE-TREATMENT')
}
